...
struct Graph {
    ...
    Graph (int n, int test) : nodes(n), testNo(test) {
        ...
        neighbors.resize(n);
        for (Edge e : edgeList) {
            neighbors[e.u].push_back(e.v);
            neighbors[e.v].push_back(e.u);
        }
    }
...
struct Solution {
    ...
    void makeOrd () {
        ...
        shuffle(begin(ord), end(ord), default_random_engine(0));
    }
    
    int chooseNeighbor (int u) {
        int x = randomInt(0, (int) graph.neighbors[u].size() - 1);
        return graph.neighbors[u][x];
    }
...
double singleAgent (int ag, int probs, int sim) {
    ...
    for (int prb = 0; prb < probs; prb++) {
        ...
        for (int ss = 0; ss < sim; ss++) {
            clock_t start = clock();
            sum += singleSimulation(populationSize, eliteSize, ag);
            clock_t end = clock();
            double secs = ((double) end - (double) start) / CLOCKS_PER_SEC;
            printf("Agents = %d Test = %d Simulation = %d took = %f seconds\n", ag, prb, ss, secs);
        }
        clock_t prEnd = clock();
...
void getSolution () {
    ...
    for (int i = 0; i < (int) ans.size(); i++) {
        printf("mod Agents %d Utility = %f diff in per. = %f\n", (i + 1) * 5, ans[i], ((ans[i] - z[i]) * 100) / z[i]);
    }
}
